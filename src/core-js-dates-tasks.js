...
function getNextFriday(date) {
  ...
  const daysUntilFriday = 5 - dayOfWeek + (dayOfWeek < 5 ? 0 : 7);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = Math.round((endDate - startDate) / dayMsec);
...
function getWeekNumberByDate(date) {
  ...
  const diffDays = Math.floor((propsDate - startOfYear) / dayMsec) + 1;

  return Math.ceil((diffDays + daysToFullWeek) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (start <= end) {
    if (iWork > countWorkDays) {
      iWork = 1;
      start.setDate(start.getDate() + countOffDays);
    } else {
      schedule.push(formatedDate(start));
      iWork += 1;
